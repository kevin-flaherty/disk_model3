...
'''
    ...
    def set_structure(self):
        ...
        z_grid = rotation[:,:,2]
        full_z_grid = z_grid[:,:,np.newaxis] + zf
        '''translating x &y grids back to polar coordinates'''
        r_grid, f_grid = cart2pol(rotation[:,:,0], rotation[:,:,1])
        r_full_grid = r_grid[:,:,0]+np.ones(len(zf))
        f_full_grid = f_grid[:,:,0]+np.ones(len(zf))
        ...
        ###### add angle to grid? ######
        grid = {'nac':nac,'nfc':nfc,'nzc':nzc,'rcf':r_full_grid,'amax':amax,'zcf':full_z_grid}#'ff':ff,'af':af,
        self.grid=grid
